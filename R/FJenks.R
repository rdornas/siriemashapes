      ...
      t %>%
        dplyr::mutate(
          ...
          FisherJenks3 = dplyr::case_when(
            ...
            `HS-UCL` >= Fisher3$brks[2] &
              `HS-UCL` <= Fisher3$brks[3] ~ "Medium",
            `HS-UCL` >= Fisher3$brks[3] &
              `HS-UCL` <= Fisher3$brks[4] ~ "High",
            TRUE ~ NA_character_
          ),
          ...
          FisherJenks5 = dplyr::case_when(
            `HS-UCL` >= Fisher5$brks[1] &
              `HS-UCL` <= Fisher5$brks[2] ~ "Very Low",
            `HS-UCL` >= Fisher5$brks[2] &
              `HS-UCL` <= Fisher5$brks[3] ~ "Low",
            `HS-UCL` >= Fisher5$brks[3] &
              `HS-UCL` <= Fisher5$brks[4] ~ "Medium",
            `HS-UCL` >= Fisher5$brks[4] &
              `HS-UCL` <= Fisher5$brks[5] ~ "High",
            `HS-UCL` >= Fisher5$brks[5] &
              `HS-UCL` <= Fisher5$brks[6] ~ "Very High",
            TRUE ~ NA_character_
          ),
